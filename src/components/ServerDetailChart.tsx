...
function InfoCell({ label, value, accent }: { label: string; value: string; accent?: string }) {
  return (
    <div className="min-w-0 border-b border-[#E9EEF1] py-[13px] last:border-b-0 dark:border-[#26313A] sm:[&:nth-last-child(-n+2)]:border-b-0">
      <p className="text-[11px] text-[#7A8792]">{label}</p>
      <strong className={cn("mt-[7px] block truncate text-base font-semibold tabular-nums text-[#202A33] dark:text-[#EDF3F6]", accent)}>{value}</strong>
    </div>
  )
}
  ...
  return (
    <article data-testid={`resource-realtime-${dataKey}`} className="min-w-0 overflow-hidden rounded-lg border border-[#DDE4E9] bg-white px-[22px] py-5 shadow-[0_6px_18px_rgba(32,42,51,0.035)] dark:border-[#2D3943] dark:bg-[#1A2229]">
      ...
      <div className="mt-2.5 flex min-w-0 items-center gap-3.5">
        ...
        <strong className="min-w-0 truncate text-[27px] font-semibold leading-none tabular-nums" style={{ color }}>{value}</strong>
      ...
      <div className="mt-[15px] h-1.5 overflow-hidden rounded-[3px] bg-[#E8EDF0] dark:bg-[#2B3740]">
        <span className="block h-full rounded-[3px] transition-[width] duration-500" style={{ width: `${clampPercent(percent)}%`, background: color }} />
      </div>
    </article>
  )
}
...
function ResourceHistoryCard({
  ...
  return (
    <section data-testid={`resource-history-${dataKey}`} className="min-w-0 overflow-hidden rounded-lg border border-[#DDE4E9] bg-white shadow-[0_6px_18px_rgba(32,42,51,0.035)] dark:border-[#2D3943] dark:bg-[#1A2229]">
      <header className="flex min-h-[54px] items-center justify-between gap-4 border-b border-[#E9EEF1] px-[18px] dark:border-[#26313A]">
        ...
        <Select
          ...
          aria-label={`${label} ${t("monitor.timeRange", { defaultValue: "时间范围" })}`}
      ...
      <div className="relative px-4 pb-4 pt-3">
        <div className="h-[278px] max-[620px]:h-[230px]">
          <ChartContainer config={chartConfig} className="h-full w-full">
            <ComposedChart accessibilityLayer data={chartData} margin={{ left: 4, right: 12, top: 10, bottom: 0 }}>
              ...
              <Line type="monotone" dataKey={dataKey} stroke={color} strokeWidth={2.5} dot={false} connectNulls={false} isAnimationActive={false} />
            </ComposedChart>
          </ChartContainer>
        </div>
        {!hasChartData && (
    ...
    <div className="space-y-4">
      ...
      <div className="grid items-stretch gap-4 min-[861px]:grid-cols-[.98fr_1.12fr]">
        <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-lg border border-[#DDE4E9] bg-white shadow-[0_6px_18px_rgba(32,42,51,0.035)] dark:border-[#2D3943] dark:bg-[#1A2229]">
          ...
          <div className="flex flex-1 flex-col justify-between px-[18px] pb-[18px] pt-[18px]">
            <div className="grid grid-cols-2 gap-x-6">
              <InfoCell label={t("serverDetail.remainingTraffic", { defaultValue: "剩余流量" })} value={trafficRemaining === null ? "--" : formatBytes(trafficRemaining)} accent="text-[#22B573]" />
              <InfoCell label={t("serverDetail.usedTraffic", { defaultValue: "已用流量" })} value={formatBytes(trafficUsed)} />
            </div>
            <div data-testid="traffic-usage-progress" className="relative mt-[17px] min-h-[83px] overflow-hidden rounded-lg border border-[#E9EEF1] bg-[#FBFCFD] dark:border-[#26313A] dark:bg-[#26313C]">
              <span
                aria-hidden="true"
              ...
              />
              <div className="relative z-[1] flex min-h-[83px] items-center justify-between gap-4 px-4 py-2.5">
            ...
            <div className="mt-[15px] grid grid-cols-2 gap-[18px] max-[620px]:grid-cols-1">
              <div className="flex min-w-0 items-baseline justify-between gap-2">
                <span className="text-[11px] text-[#7A8792]">{t("serverDetail.currentUpload", { defaultValue: "当前上传" })}</span>
                <strong className="truncate text-[15px] font-semibold tabular-nums">{formatSpeed(info.up)}</strong>
              </div>
              <div className="flex min-w-0 items-baseline justify-between gap-2">
                <span className="text-[11px] text-[#7A8792]">{t("serverDetail.currentDownload", { defaultValue: "当前下载" })}</span>
                <strong className="truncate text-[15px] font-semibold tabular-nums">{formatSpeed(info.down)}</strong>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-lg border border-[#DDE4E9] bg-white shadow-[0_6px_18px_rgba(32,42,51,0.035)] dark:border-[#2D3943] dark:bg-[#1A2229]">
          ...
          <div className="grid flex-1 auto-rows-fr grid-cols-2 gap-x-6 px-[18px] pb-[18px] pt-[18px]">
            ...
            <InfoCell label={t("serverDetail.lastActive")} value={info.last_active_time_string || "--"} />
          </div>
        </section>
      </div>

      <section aria-labelledby="resource-history-title" className="space-y-3.5">
        ...
        <div className="grid grid-cols-1 gap-4 min-[861px]:grid-cols-3">
          <ResourceHistoryCard label="CPU" dataKey="cpu" hours={hours} chartData={chartData} isLoading={isPending} onHoursChange={setHours} />
          <ResourceHistoryCard label={t("serverDetail.mem")} dataKey="memory" hours={hours} chartData={chartData} isLoading={isPending} onHoursChange={setHours} />
          <ResourceHistoryCard label={t("serverDetail.disk")} dataKey="storage" hours={hours} chartData={chartData} isLoading={isPending} onHoursChange={setHours} />
        </div>
      </section>
    </div>
  )
