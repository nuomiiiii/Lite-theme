...
test("uses one resolved server ID for overview, realtime charts and ping charts", () => {
  ...
  assert.match(tabSwitch, /from "@mui\/material\/Tabs"/)
  assert.match(tabSwitch, /setCurrentTab\(value\)/)
  ...
  assert.match(realtime, /TrafficStatCard/)
  ...
  assert.match(realtime, /grid-cols-3/)
  assert.match(realtime, /min-\[1101px\]:grid-cols-3/)
