...
import {
  isLiteServerOnline,
  leftoverStatusUuids,
  resolveLiteServerEntries,
} from "../src/lib/lite-node-list.ts"
...
test("keeps registered offline servers when live status omits them", () => {
  ...
  const entries = resolveLiteServerEntries(nodes, latest)
  assert.deepEqual(
    entries.map((entry) => [entry.node.name, isLiteServerOnline(entry.status)]),
    [
      ["online-1", true],
...
test("does not drop the offline machine after a partial live status snapshot", () => {
  ...
  const entries = resolveLiteServerEntries(nodes, latest)
  const onlineCount = entries.filter((entry) => isLiteServerOnline(entry.status)).length
...
test("empty live status still lists every registered node as offline", () => {
  ...
  const entries = resolveLiteServerEntries(nodes, {})
  assert.equal(entries.length, 2)
  assert.ok(entries.every((entry) => !isLiteServerOnline(entry.status)))
})
...
test("home poll refreshes the node list so deleted servers leave with getNodes", () => {
  ...
  assert.match(provider, /getLiteNodes\(false\)/)
  assert.match(utils, /getLiteNodes\(true\)/)
  assert.doesNotMatch(utils, /statusMap\.has\(server\.uuid\)/)
  assert.doesNotMatch(utils, /liteServerCache\.filter/)
  assert.match(utils, /resolveLiteServerEntries\(nodes, data\)/)
})
